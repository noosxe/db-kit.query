	...
	if ($this.flags.allColumns) {
		...
		_.forEach($this.columns, function(as, column) {
			if (as === column) {
				...
				column = _.map(column.split('.'), function(colPart) {
					return quote(colPart);
				}).join('.');
		...
		switch(op.type) {
			case 'operation':
				...
				if (prepared) {
					...
					q += (_.isString() ? quotev(op.value) : op.value);
				}
				break;
			case 'connector':
				q += ' ' + op.operator + ' ';
...
Query.prototype._genJoin = function() {
	...
	_.forEach($this._joins, function(join, index) {
		...
		_.forEach(on, function(op) {
			switch(op.type) {
				case 'operation':
					var column = _.map(op.column.split('.'), function(colPart) {
						return quote(colPart);
					}).join('.');
					...
					if (op.value.indexOf('.') == -1) {
						_column = _.isString(op.value) ? quotev(op.value) : op.value;
					} else {
						_column = _.map(op.value.split('.'), function(colPart) {
							return quote(colPart);
						}).join('.');
					}
