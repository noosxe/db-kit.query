...
Query.prototype._genSelectSub = function(prepared,values) {
	var $this = this;
...
Query.prototype._genSelect = function(prepared) {
	...
	q += 'FROM ' + ($this._joins.length > 0 ? $this._genSelectSub(prepared, values) : quote($this.tableName));
	...
	if ($this._joins.length === 0 && $this._where.length > 0) {
		q += ' ' + $this._genWhere(prepared, values);
	}

	if ($this._joins.length === 0 && $this._orderBy.length > 0) {
		var order = [];
	...
	if ($this._joins.length === 0 && $this._limit !== null) {
		q += ' LIMIT ';
		if (prepared) {
			q += '?';
			values.push($this._limit);
	...
	if ($this._joins.length === 0 && $this._offset !== null) {
		q += ' OFFSET ';
		if (prepared) {
			q += '?';
			values.push($this._offset);
...
Query.prototype._genCreateTable = function(prepared) {
	...
	cols = cols.concat($this._genIndexesAndConstraints());
...
/**
 * Internal "join ..." query part generator
 * @returns {string}
 * @private
 */
...
/**
 * Internal indexes and constraints query part generator
 *
 * @returns {Array}
 * @private
 */
Query.prototype._genIndexesAndConstraints = function() {
	var results = [];
	...
	_.forEach(this.tableColumns, function(columnDescription, name) {
		if (columnDescription.primary) {
			pk.push('PRIMARY KEY (' + quote(name) + ')');
			return;
		}

		if (columnDescription.unique) {
			unique.push('UNIQUE KEY ' + quote('u_' + name) + ' (' + quote(name) + ')');
			return;
		}

		if (columnDescription.index) {
			index.push('KEY ' + quote('k_' + name) + ' (' + quote(name) + ')');
		}
	});

	if (this.pkFields && this.pkFields.length > 0) {
		pk.push('PRIMARY KEY (' + _.map(this.pkFields, function(f) { return quote(f) }).join(',') + ')');
	}

	_.forEach(this.references, function(ref, column) {
		index.push('KEY ' + quote('k_' + column) + ' (' + quote(column) + ')');
		constraint.push('CONSTRAINT ' + quote(this.tableName + '_fk_' + ref.table) + ' FOREIGN KEY (' + quote(column) + ') REFERENCES ' + quote(ref.table) + ' (' + quote(ref.field) + ')');
	}.bind(this));

	return results.concat(pk, unique, index, constraint);
};
...
Query.prototype.addReference = function(ref) {
	this.references[ref.column] = ref;

	return this;
};

Query.prototype.setPrimaryKeyFields = function(pkf) {
	this.pkFields = pkf;
